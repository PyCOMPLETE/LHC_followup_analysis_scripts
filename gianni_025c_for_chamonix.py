...
folders = ['../LHC_2016_25ns_beforeTS1/', '/afs/cern.ch/project/spsecloud/LHC_2015_PhysicsAfterTS2/', '/afs/cern.ch/project/spsecloud/LHC_2015_PhysicsAfterTS3/',
...
for key in hl_keys:
    ...
    sphl1.plot(main_dict['filln'], main_dict[moment]['heat_load']['arc_averages'][key], '.', label=key, color=color, markersize=12)
    sphl1.grid('on')

    sphl2.plot(main_dict['filln'], main_dict[moment]['heat_load']['arc_averages'][key]/main_dict[moment]['intensity']['total'], '.', label=key, color=color, markersize=12)
    sphl2.grid('on')
...
for i_sec, sec in enumerate(sector_list):
    hl_mat.append(main_dict[moment]['heat_load']['arc_averages'][sec]/divide_by)
    int_hl_mat.append( main_dict['hl_integrated']['arc_averages'][sec])
...
for i_sec, sec in enumerate(sector_list):
    ...
    for temp in [spc_hl15, spc_hl16]:
        temp.plot(tc(main_dict['t_start_fill']), main_dict[moment]['heat_load']['arc_averages'][sec]/divide_by, '.', label=sec, color=color, markersize=12)
...
for ctr, bin_ in enumerate(bins):
    ...
    sp.plot(np.cumsum(bin_int_hl), bin_hl/tot_int, '.', color=color, label=label, markersize=12)

tot_hl = tot_arr / tot_divisor
tot_int_hl = tot_int_arr / tot_int_divisor
sp.plot(np.cumsum(tot_int_hl), tot_hl/tot_int, '.', color='black', label='Average', markersize=12)
...
figi2 = pl.figure(101, figsize = (8*1.5,6*1.5))
figi2.set_facecolor('w')
spi1 = pl.subplot2grid((4,1),(0,0))
pl.plot(np.cumsum(avg_int_hl), main_dict[moment]['n_bunches']['b1'], '.', color='b', markersize=12)
spi2 = pl.subplot2grid((4,1),(1,0), rowspan=3, sharex = spi1)
sp = spi2
...
for i_sec, sec in enumerate(sector_list):
    color = colorprog(i_sec, 8)
    #spi2.plot(np.cumsum(avg_int_hl), main_dict[moment]['heat_load']['arc_averages'][sec]/divide_by, '.', label=sec, color=color, markersize=12)
    spi2.plot(np.cumsum(int_hl_mat[i_sec]), main_dict[moment]['heat_load']['arc_averages'][sec]/divide_by, '.', label=sec, color=color, markersize=12)
