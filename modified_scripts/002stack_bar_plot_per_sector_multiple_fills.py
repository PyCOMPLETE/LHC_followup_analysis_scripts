...
for i, s in enumerate(hl.sector_list()):
    ...
    for i_snapshot in xrange(N_snapshots):
        ...
        if i_snapshot in st_ind:
            iisnp = st_ind[0]
            ax1_sect.bar(ind-0.32, hl_cells, 0.3,bottom = zero_level,
                         color=colorlist[i_snapshot], alpha=.5)
            zero_level += hl_cells
        else:
            ax1_sect.bar(ind+0.02, hl_cells, 0.3,
                       color=colorlist[i_snapshot], alpha=.5)
...
for i, s in enumerate(hl.sector_list()):
    ...
    for i_snapshot in xrange(N_snapshots):
        if i_snapshot in st_ind:
            iisnp = [2*(i+1)-0.5]
            axviol2.bar(iisnp, [np.nanmean(snapshots[i_snapshot]['dict_hl_cell_by_cell'][s]['heat_loads'])/totintnorm],0.45,bottom=zero_level,color=colorlist[i_snapshot], alpha=.5)
            zero_level += np.nanmean(snapshots[i_snapshot]['dict_hl_cell_by_cell'][s]['heat_loads'])/totintnorm
        else:
            axviol2.bar([2*(i+1)], [np.nanmean(snapshots[i_snapshot]['dict_hl_cell_by_cell'][s]['heat_loads'])/totintnorm],0.45,0,color=colorlist[i_snapshot], alpha=.5)
