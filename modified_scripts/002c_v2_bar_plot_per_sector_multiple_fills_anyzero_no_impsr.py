import sys, time, os
#sys.path.append("../")
#sys.path.append("../../_analysis_scripts/")
...
parser.add_argument('--no_imp_SR', help='Subtract SR and Impedance',action='store_true')
...
parser.add_argument('--zeroat', help='Heat load offset time, format dd-mm-yyyy,hh:mm', default=None)
...
for i_snapshot in xrange(N_snapshots):
    ...
    # find offset from --zeroat to remove
    if zero_at is not None:
        ...
        if ':' in zero_at:
            ...
            hid_offset = qf.get_fill_dict(filln_offset)
            t_fill_st_offset = dict_fill_bmodes[filln_offset]['t_startfill']
            t_ref_offset=t_fill_st_offset

            t_sample_h_offset = (t_zero_unix - t_ref_offset)/3600

            dict_hl_cell_by_cell_offset = cch.sample_and_sort_cell_by_cell(hid_offset, t_ref=t_ref_offset, t_sample_h=t_sample_h_offset, t_offset_h=None, first_cell=args.first_cell)
            if args.no_imp_SR:
                srimp_subtr = hl_imped_sample+hl_sr_sample
            else:
                srimp_subtr = 0.
            for key in dict_hl_cell_by_cell.keys():
                    dict_hl_cell_by_cell[key]['heat_loads'] = dict_hl_cell_by_cell[key]['heat_loads'] - dict_hl_cell_by_cell_offset[key]['heat_loads']-srimp_subtr
            print('Offsets subtracted')
        else:
            raise ValueError('--zeroat must be in the format dd-mm-yyyy,hh:mm')
...
plt.close('all')
myfontsz = 16
ms.mystyle_arial(fontsz=16,dist_tick_lab=3)
if N_snapshots==1:
    width = 0.6
else:
    width = 0.8
...
for i, s in enumerate(hl.sector_list()):
    ...
    #single sector plot
    fig_sect = plt.figure(1000+i, figsize=(15,8.5), tight_layout=False)
    ...
    for i_snapshot in xrange(N_snapshots):
        ...
        fig_sect.subplots_adjust(left=.06, right=.96, top=0.9, hspace=0.37, bottom=0.07)
    ...
    if zero_at is not None:
        to_table.append(['T_nobeam [h]'] + [zero_at for i_snapshot in xrange(N_snapshots)])
    else:
...
for i, s in enumerate(hl.sector_list()):
    ...
    for i_snapshot in xrange(N_snapshots):
        if N_snapshots % 2 == 0:
              bar_starts = np.arange(-N_snapshots/2*(lll+gap)+gap/2,N_snapshots/2*(lll+gap)-gap/2,lll+gap)
        else:
              bar_starts = np.arange(-N_snapshots/2*lll + gap,N_snapshots/2*lll+gap,lll+gap)
        axviol2.bar([2*(i+1)+bar_starts[i_snapshot]], [np.nanmean(snapshots[i_snapshot]['dict_hl_cell_by_cell'][s]['heat_loads'])/totintnorm],lll,0,color=colorlist[i_snapshot], alpha=.5)
...
for i_snapshot in xrange(N_snapshots):
    ...
    avHL = []
    for s in hl.sector_list(): 
        avHL.append(np.nanmean(snapshots[i_snapshot]['dict_hl_cell_by_cell'][s]['heat_loads']))
    savHL.append(sum(avHL))
             
zero_level = 0.0
for i_snapshot in xrange(N_snapshots): 
    axviol2.bar([2*2*(N_snapshots+2)+bar_starts[i_snapshot]], savHL[i_snapshot]/8./totintnorm,lll,0,color=colorlist[i_snapshot], alpha=.5)
    
axviol2.grid('on')

#############################
axviol2.set_xlim([0,22])
axviol2.set_xticks(2*(np.arange(11)+1))
axviol2.set_xticklabels(['Arc %d'%s for s in hl.sector_list()]+['','LHC'])
axviol2.set_ylim(-0.1e-13,3e-13)
...
if args.o:
    ...
    for i_snapshot, snapshot in enumerate(snapshots):
        str_file += 'fill%dat%.0fh_'%(snapshot['filln'], snapshot['t_h']*100)
    ...
    figviol2.savefig(folname+'/cellbycell_%s_%s_arcHL.png'%(str_file, tagfname), dpi=200)
