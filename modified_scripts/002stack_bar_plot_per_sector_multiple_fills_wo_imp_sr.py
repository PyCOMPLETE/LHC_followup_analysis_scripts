...
parser.add_argument('--stackwho', help='Stacks bars', default=[0,1])
...
spshare = None
spsharehist = None
colorlist = ['b','r','g', '#9500FF', 'm', 'k']
colorleglist = ['#7F7FFF', '#FF7F7F', '#7FBF7F','#9500FF']
...
for i, s in enumerate(hl.sector_list()):
    ...
    #single sector plot
    fig_sect = plt.figure(1000+i, figsize=(16,8.5), tight_layout=False)
    ...
    for i_snapshot in xrange(N_snapshots):
        ...
        #ax1_sect.bar(ind-width/2+i_snapshot*width/N_snapshots, hl_cells, width/N_snapshots,bottom = zero_level,
        #    color=colorlist[i_snapshot], alpha=.5)

        if args.stackwho:
            st_ind = map(int,[x for x in args.stackwho if x.isdigit()])

        if i_snapshot in st_ind:
            iisnp = st_ind[0]
            ax1_sect.bar(ind-0.32, hl_cells-snapshots[i_snapshot]['hl_imped_sample']-snapshots[i_snapshot]['hl_sr_sample'], 0.3,bottom = zero_level,
                         color=colorlist[i_snapshot], alpha=.5)
            zero_level += hl_cells-snapshots[i_snapshot]['hl_imped_sample']-snapshots[i_snapshot]['hl_sr_sample']
        else:
            ax1_sect.bar(ind+0.02, hl_cells-snapshots[i_snapshot]['hl_imped_sample']-snapshots[i_snapshot]['hl_sr_sample'], 0.3,
                       color=colorlist[i_snapshot], alpha=.5)
        ...
        # histogram
        histval, binedges = np.histogram(hl_cells-snapshots[i_snapshot]['hl_imped_sample']-snapshots[i_snapshot]['hl_sr_sample'], bins=nbinhist, range=(minhist, maxhist))
...
for i_snapshot, col_snsh, sign_shsh in zip([0,1,2], ['b', 'r','g'], [-1., -1., 1.]):
...
if normtointen:
    axviol.set_ylabel('Norm. heat load [W/half-cell/p+]')
else:
...
# Arcs stack plot
#plt.close(1);
figviol2 = plt.figure(2, figsize=(12,8.5))
figviol2.set_facecolor('w')
axviol2 = plt.subplot2grid((2,3), (0,0), colspan=3)
maxdistr = np.max(np.array(y_list)[:])
#for i_snapshot, col_snsh, sign_shsh in zip([0,1,2], ['b', 'r','g'], [-1., -1.,1.]):
...
for i, s in enumerate(hl.sector_list()):
    ...
    for i_snapshot in xrange(N_snapshots):
        if i_snapshot in st_ind:
            iisnp = [2*(i+1)-0.5]
            axviol2.bar(iisnp, [(np.nanmean(snapshots[i_snapshot]['dict_hl_cell_by_cell'][s]['heat_loads'])-snapshots[i_snapshot]['hl_imped_sample']-snapshots[i_snapshot]['hl_sr_sample'])/totintnorm],0.45,bottom=zero_level,color=colorlist[i_snapshot], alpha=.5)
            zero_level += (np.nanmean(snapshots[i_snapshot]['dict_hl_cell_by_cell'][s]['heat_loads'])-snapshots[i_snapshot]['hl_imped_sample']-snapshots[i_snapshot]['hl_sr_sample'])/totintnorm
        else:
            axviol2.bar([2*(i+1)], [(np.nanmean(snapshots[i_snapshot]['dict_hl_cell_by_cell'][s]['heat_loads'])-snapshots[i_snapshot]['hl_imped_sample']-snapshots[i_snapshot]['hl_sr_sample'])/totintnorm],0.45,0,color=colorlist[i_snapshot], alpha=.5)



#axviol2LHC = axviol2.twinx()
############# FOR LHC bar
savHL = []
for i_snapshot in xrange(N_snapshots):
    ...
    avHL = []
    for s in hl.sector_list():
        avHL.append(np.nanmean(snapshots[i_snapshot]['dict_hl_cell_by_cell'][s]['heat_loads'])-snapshots[i_snapshot]['hl_imped_sample']-snapshots[i_snapshot]['hl_sr_sample'])
    savHL.append(sum(avHL))
...
for i_snapshot in xrange(N_snapshots):
    ...
    if i_snapshot in st_ind:
        iisnp = [2*(i+2+1)-0.5]
        axviol2.bar(iisnp, savHL[i_snapshot]/8./totintnorm,0.45,bottom=zero_level,color=colorlist[i_snapshot], alpha=.5)
        zero_level += savHL[i_snapshot]/8./totintnorm
    else:
        axviol2.bar([2*(i+2+1)], savHL[i_snapshot]/8./totintnorm,0.45,0,color=colorlist[i_snapshot], alpha=.5)

axviol2.grid('on')

#############################
axviol2.set_xticks(2*(np.arange(11)+1))
axviol2.set_xticklabels(['Arc %d'%s for s in hl.sector_list()]+['','LHC'])
...
#axviol2.set_ylim(min_hl_scale, max_hl_scale)
axviol2.set_xlim(1,22)
...
figviol2.suptitle('%s'%({False:'recalc. values', True:'DB values'}[from_csv]))
...
if args.o:
    ...
    figviol2.savefig(folname+'/cellbycell_%s_%s_distrib_bar.png'%(str_file, tagfname), dpi=200)
