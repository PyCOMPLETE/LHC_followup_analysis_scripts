...
lg = ms.comb_legend(ax1,ax11,loc ='upper left', prop={'size':fontsz_leg}, frameon=False)
...
ax2.legend(ncol = 3, columnspacing = 1, loc='upper left', prop={'size':fontsz_leg}, frameon=False)
#ax2.set_ylim(0, None)
ax2.grid('on')
...
if time_in == 'd' or time_in == 'h':
    ax2.set_xlabel('Time [%s]' % time_in)

pl.suptitle('From ' + tref_string)
fig.subplots_adjust(left=.12, right=.76, hspace=.28, top=.87,bottom = 0.22)
